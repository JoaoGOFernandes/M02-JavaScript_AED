...
*/
 ...
 //age está definido apenas no escopo de bloco da função showName, não podendo ser acessada no escopo global
/*
  03
...
*/
 const car = {
    name: 'Ralampago Maquinhos',
    brand: 'Mazda RX8',
    color: ['vermelho', 'preto', 'azul'],
    isRunning: false,
    run () {
        this.isRunning = true
    ...
    getColorsMessage () {
        const colors = this.color.join(', ').replace(`, ${this.color[2]}`, ` e ${this.color[2]}`)
        console.log(`O ${this.name} está disponivel nas cores ${colors}`);
    }
 }
